...
flowchart TD
    ...
    J -->|Manual entry| K["Enter bank-displayed beneficiary name and payment destination"]
    K --> O
    ...
    O --> P["Classify each relationship as Coincide, No coincide, or No verificado"]
    P --> Q["Order results: No coincide, No verificado, Coincide"]
    Q --> R["Explain sources, uncertainty, and appropriate next step"]
    R --> S["User makes the payment decision outside the prototype"]
```
